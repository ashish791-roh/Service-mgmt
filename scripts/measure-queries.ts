...
async function run() {
  ...
  await measure('InventoryItem raw low stock count query', () => 
    prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM "InventoryItem" WHERE "branchId" = $1 AND "quantity" <= "minQuantity"`, branchId)
  );
  
  await measure('Job raw revenue query', () => 
    prisma.$queryRawUnsafe(`SELECT SUM(COALESCE("actualCost", COALESCE("estimatedCost", 0)))::float as sum FROM "Job" WHERE "branchId" = $1 AND "status" IN ('Completed', 'Delivered')`, branchId)
  );
  
  await measure('Sale raw revenue query', () => 
    prisma.$queryRawUnsafe(`SELECT SUM(COALESCE("totalAmount", 0))::float as sum FROM "Sale" WHERE "branchId" = $1`, branchId)
  );
...
async function measure(name: string, queryFn: () => Promise<any>) {
  ...
  try {
    ...
    console.log(`  x ${name.padEnd(55)} : Failed with error: ${err.message}`);
  }
}

async function printCount(table: string) {
  try {
    const prismaClientAny = prisma as any;
    const count = await prismaClientAny[table.charAt(0).toLowerCase() + table.slice(1)].count();
