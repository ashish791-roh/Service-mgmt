...
export const TallyIntegrationPage: React.FC = () => {
  ...
  // Phase 3 States
  const [currentTab, setCurrentTab] = useState<'dashboard' | 'queue' | 'ocr' | 'settings'>('queue');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6">
      {/* Top Header Card */}
      <div className="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">
            Monitor real-time event syncs, manage the prioritized task execution queue, and run side-by-side OCR review tools.
          </p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/40 relative">
          <button
            ...
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 z-10 ${
              currentTab === 'dashboard' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            <Layers size={14} />
            Dashboard
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 z-10 ${
              currentTab === 'queue' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            <FileText size={14} />
            Queue
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 z-10 ${
              currentTab === 'ocr' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            <Upload size={14} />
            OCR Parser
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 z-10 ${
              currentTab === 'settings' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            <Sliders size={14} />
            Settings
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid gap-3 grid-cols-2 lg:grid-cols-3">
              {statsList.map(item => {
                ...
                return (
                  <div key={item.label} className="rounded-2xl border border-slate-100 bg-slate-50/40 p-4 space-y-2 flex flex-col justify-between hover:bg-slate-50 transition-all shadow-sm">
            ...
            {/* Accuracy rate details */}
            <div className="bg-slate-50/40 border border-slate-100 rounded-2xl p-5 space-y-3">
              <div className="flex justify-between items-center text-xs font-semibold text-slate-700">
                <span className="flex items-center gap-2 text-slate-700">
                  <CheckCircle size={14} className="text-teal-600" />
                  OCR Extraction Success Rate
                </span>
                <span className="text-purple-700 font-bold text-sm">{stats?.successRate ?? 0}%</span>
              </div>
              <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                <div
                  className="bg-gradient-to-r from-teal-500 via-purple-500 to-indigo-500 h-full rounded-full transition-all duration-500"
                  style={{ width: `${stats?.successRate ?? 0}%` }}
                />
              </div>
              <p className="text-[11px] text-slate-650">
                Calculated based on correct fields approved and auto-synced into TallyPrime ledgers vs manually corrected entries.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center p-3 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                  <span className="font-semibold text-slate-500">Sync Status</span>
                  <div className="flex items-center gap-2">
                    {settings.syncStatus === 'ready' && (
            ...
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm text-xs text-slate-600 flex gap-3">
      ...
      {currentTab === 'queue' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-4 shadow-sm space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-4">
            {/* Search bar */}
                ...
                {filteredDocuments.map(doc => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-4 hover:border-slate-300 transition-colors"
                    >
                      {/* Card Header */}
                      ...
                      <div className="grid gap-3 grid-cols-2 md:grid-cols-4 bg-slate-50/50 rounded-2xl border border-slate-100 p-4 text-xs">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-slate-800 mt-1 truncate max-w-[155px]">{doc.extractedData?.customerName || doc.extractedData?.supplierName || '—'}</p>
                        </div>
                      </div>
                        ...
                        <div className="border-t border-slate-100 pt-4 mt-3 space-y-4">
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            <label className="block">
                              ...
                              <input
                                ...
                                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-800 outline-none focus:border-teal-500 transition-colors"
                              />
                            </label>
                            <label className="block">
                            ...
                            <label className="block">
                              ...
                              <input
                                ...
                                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-800 outline-none focus:border-teal-500 transition-colors"
                              />
                            </label>
                            <label className="block">
                            ...
                            <label className="block">
                              ...
                              <input
                                ...
                                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-mono font-bold text-slate-800 outline-none focus:border-teal-500 transition-colors"
                              />
                            </label>
                            <label className="block">
                            ...
                            <label className="block sm:col-span-2">
                              ...
                              <input
                                ...
                                className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-900 outline-none focus:border-teal-500 transition-colors bg-teal-50/10"
                              />
                            </label>
                          </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                                {editingData.items.map((item: any, idx: number) => (
                                  <div key={idx} className="p-3 border border-slate-200 rounded-2xl bg-white grid gap-2 sm:grid-cols-[2.5fr_1fr_1.25fr_1.5fr_1.25fr] items-end">
                                    <label className="block">
                          ...
                          <div className="flex gap-2 justify-end border-t border-slate-100 pt-3">
                            <button
                              ...
                              className="px-4 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-600 bg-white hover:bg-slate-50 transition-colors"
                            >
                              Cancel
                            </button>
                            <button
      ...
      {currentTab === 'ocr' && (
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 className="font-bold text-base text-slate-800 flex items-center gap-2 pb-3 border-b border-slate-100">
              <Upload size={18} className="text-teal-600" />
              Upload Document for OCR Analysis
            </h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Upload invoices, purchase bills, receipts, or other documents to run AI layout parsing and schema validation. Extracted details will be queued automatically for review before syncing to Tally ERP.
            </p>
          </div>
          ...
          <div
            ...
            onDrop={handleDrop}
            className={`relative rounded-3xl border-2 border-dashed p-10 text-center transition-all ${dragActive
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center gap-3">
              <div className={`p-5 rounded-full bg-teal-50 text-teal-600 border border-teal-100 transition-transform ${dragActive ? 'scale-110' : ''}`}>
                <Upload size={28} />
              </div>
              <div>
              ...
              <div className="mt-4 flex items-center gap-2 bg-slate-100 p-2 rounded-xl border border-slate-200/40 text-xs">
                <span className="px-2 text-slate-500 font-semibold">Classification:</span>
                <select
                  value={documentType}
                  onChange={e => setDocumentType(e.target.value)}
                  className="bg-white rounded-lg border border-slate-200 px-3 py-1 font-semibold text-slate-700 outline-none cursor-pointer focus:border-teal-500"
                >
              ...
              {uploading && (
                <div className="absolute inset-0 bg-white/95 rounded-3xl flex items-center justify-center backdrop-blur-[2px]">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-8 h-8 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" />
                    <p className="text-xs font-bold text-slate-700 animate-pulse">Running OCR & parsing schema...</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {uploadResult && (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs font-semibold text-slate-700 flex items-center justify-between gap-3 shadow-sm">
      ...
      {currentTab === 'settings' && (
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] items-start text-slate-800">
          {/* Settings Panel */}
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-6 text-slate-800">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-4 bg-slate-50/50 col-span-2 flex items-center justify-between shadow-sm">
                <div className="space-y-0.5">
                  <span className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Enable Background Synchronization</span>
                  <span className="block text-[11px] text-slate-500">Keep data synchronized automatically with Tally ERP</span>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-bold w-16 ${settings.enabled ? 'text-teal-600' : 'text-slate-500'}`}>
              ...
              <div className="block col-span-2">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-900 placeholder:text-slate-400 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-semibold shadow-sm"
                  placeholder="Tally company name (exact casing)"
                />
              </div>

              <div className="block">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block">Tally host</label>
                <input
                  value={settings.host}
                  onChange={e => updateField('host', e.target.value)}
                  className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-900 placeholder:text-slate-400 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-mono font-semibold shadow-sm"
                  placeholder="localhost"
                />
              </div>

              <div className="block">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider block">Tally port</label>
                <input
                  type="number"
                  min={1}
                  value={settings.port}
              ...
              <div className="block col-span-2 border border-slate-200 p-4 rounded-2xl bg-slate-50/30">
                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block mb-3">Features & Simulation</span>
                <div className="space-y-3 text-xs font-semibold text-slate-700">
                  <label htmlFor="mock-mode-settings" className="flex items-center gap-3 cursor-pointer select-none">
                    <input
                      id="mock-mode-settings"
                      type="checkbox"
                      checked={!!settings.mockMode}
                      onChange={e => updateField('mockMode', e.target.checked)}
                      className="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500 focus:ring-offset-0 cursor-pointer"
                    />
                    <span className="text-slate-700">Enable mock Tally push (Simulate success without local ERP running)</span>
                  </label>
                  <label htmlFor="auto-push-settings" className="flex items-center gap-3 cursor-pointer select-none">
                    <input
                      ...
                      className="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500 focus:ring-offset-0 cursor-pointer"
                    />
                    <span className="text-slate-700">Automatically push to TallyPrime when approving documents</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-4 border-t border-slate-100">
              <button
                ...
                onClick={handleSave}
                disabled={saving}
                className="inline-flex items-center gap-2 rounded-xl bg-teal-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60 transition-all active:scale-[0.98]"
              >
                Save configurations
              </button>
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60 transition-all active:scale-[0.98]"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              <h3 className="font-bold text-sm text-slate-800 mb-4 pb-3 border-b border-slate-100 flex items-center gap-2">
              ...
              <div className="grid gap-3 grid-cols-2">
                {statsList.map(item => {
                  const Icon = item.icon;
                  return (
                    <div key={item.label} className="rounded-2xl border border-slate-100 bg-slate-50/40 p-4 space-y-2 flex flex-col justify-between hover:bg-slate-50 transition-all">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{item.label}</span>
                        <div className={`p-1.5 rounded-lg border ${item.color}`}>
                          <Icon size={12} />
