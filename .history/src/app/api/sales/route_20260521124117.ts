...
export async function POST(request: Request) {
    ...
    try {
        ...
        const lineData = body.items.map((li: any) => {
            ...
            // Allow admin to override unit price; reception always uses inventory price
            let unitPrice: number;
            if (auth.user.role === 'admin' && li.unitPrice !== undefined && li.unitPrice !== null) {
                const overridePrice = parseFloat(li.unitPrice);
                unitPrice = Number.isFinite(overridePrice) && overridePrice >= 0
                    ? overridePrice
                    : Number(inv.unitPrice);
            } else {
...
// ── PATCH /api/sales — mark a sale as paid (admin or reception) ──
// Body: { saleId: string }
export async function PATCH(request: Request) {
    ...
    try {
        ...
        // Check sale exists and is not already paid
        ...
        await prisma.$executeRawUnsafe(
            `UPDATE "Sale" SET "paidAt" = NOW(), "updatedAt" = NOW() WHERE "id" = $1`,
            body.saleId
        );
        ...
        return NextResponse.json(mapSale(updated, items));
    } catch (error) {
