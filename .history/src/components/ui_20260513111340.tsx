...
import { getSLAStatus, loadSLATiers, type SLATier, type SLAStatus } from '../lib/sla';
...
/**
 * Classify a job's urgency level.
 * When deviceType is provided the SLA tier thresholds are used.
 * Falls back to legacy 5/10-day thresholds when deviceType is omitted.
 */
export type JobAgeLevel = 'red' | 'yellow' | 'green';

export function getJobAgeLevel(
  createdAt: string,
  status: JobStatus,
  deviceType?: string
): JobAgeLevel {
  ...
  if (deviceType) {
    ...
    const sla = getSLAStatus(createdAt, status, deviceType, tiers);
    if (sla.level === 'breached') return 'red';
    if (sla.level === 'warning')  return 'yellow';
    return 'green';
  }

  // Legacy fallback (no device type known)
...
// ── SLABadge ─────────────────────────────────────────────────────
// Compact inline badge showing SLA status + time remaining/overdue.
// Use wherever a job row or card needs an SLA indicator.
export const SLABadge: React.FC<{
  ...
  tiers?: SLATier[];
  compact?: boolean;
}> = ({ createdAt, status, deviceType, tiers, compact = false }) => {
  const resolvedTiers = tiers ?? loadSLATiers();
  const sla: SLAStatus = getSLAStatus(createdAt, status, deviceType, resolvedTiers);

  if (sla.level === 'ok' || TERMINAL_STATUSES.includes(status)) return null;
  ...
  const s = styles[sla.level];

  return (
    <span
      className={`inline-flex items-center gap-1 border rounded-full font-semibold leading-none
        ${compact ? 'text-[10px] px-1.5 py-0.5' : 'text-[11px] px-2 py-1'}
        ${s.wrap}`}
      title={`SLA: ${sla.label} · Tier: ${sla.tier.deviceType} (${sla.tier.criticalHours}h)`}
    >
      {s.icon}
      {sla.label}
    </span>
...
// ── SLAProgressBar ────────────────────────────────────────────────
// Thin horizontal bar showing elapsed vs total SLA time.
// Use inside job cards / drawers for richer context.
export const SLAProgressBar: React.FC<{
  createdAt: string;
  status: JobStatus;
  deviceType?: string;
  tiers?: SLATier[];
}> = ({ createdAt, status, deviceType, tiers }) => {
  const resolvedTiers = tiers ?? loadSLATiers();
  const sla: SLAStatus = getSLAStatus(createdAt, status, deviceType, resolvedTiers);
  const isTerminal = TERMINAL_STATUSES.includes(status);

  const pct = Math.min(100, (sla.hoursElapsed / sla.tier.criticalHours) * 100);

  const barColor =
    isTerminal         ? 'bg-emerald-400'
    : sla.level === 'breached' ? 'bg-red-500'
    : sla.level === 'warning'  ? 'bg-amber-400'
    :                            'bg-teal-400';

  const label = isTerminal ? 'Closed' : sla.label;
  const tierLabel = `${sla.tier.deviceType} · ${sla.tier.criticalHours}h SLA`;
  ...
  return (
    <div className="w-full space-y-1">
      <div className="flex items-center justify-between text-[11px] font-medium">
        ...
        <span className={
          sla.level === 'breached' ? 'text-red-600 font-semibold'
          : sla.level === 'warning' ? 'text-amber-600'
          : 'text-gray-500'
        }>
          {label}
        </span>
...
export const PartStatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const map: Record<string, string> = {
    ...
    'AwaitingStock': 'bg-purple-500/10 text-purple-300 border-purple-500/20',
  };
