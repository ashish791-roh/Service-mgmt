...
import { StatusBadge, JobAgeBadge, getJobAgeLevel, Toast, useToast } from './ui';
import { X, CheckCircle, Clock, User, ShieldAlert, Star, Activity, AlertCircle } from 'lucide-react';
import type { ChecklistItem } from '../types';

export const JobDrawer = ({ jobId, onClose }: { jobId: string, onClose: () => void }) => {
  const { jobs, customers, devices, users, currentUser, updateJobStatus } = useApp();
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {ageLevel === 'red' && job.status !== 'Completed' && job.status !== 'Delivered' && (
            <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-4">
              ...
              <div>
                <h4 className="text-[13px] font-medium text-red-800">SLA Violation Alert</h4>
                <p className="text-[11px] text-red-600 mt-1">This job has been pending for over 10 days. Immediate attention is required to meet service level agreements.</p>
              </div>
            </div>
          )}
          {ageLevel === 'yellow' && job.status !== 'Completed' && job.status !== 'Delivered' && (
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-600 mt-1">This job is approaching the SLA threshold. Please prioritize.</p>
              </div>
            </div>
          )}
