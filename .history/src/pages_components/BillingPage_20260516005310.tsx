  ...
  <div style="margin-top:24px;">
    <div style="display:flex;justify-content:flex-end;">
      <div style="width:280px;">
        ...
        ${advanceAmount > 0 ? `
        <div style="display:flex;justify-content:space-between;padding:7px 0;font-size:13px;color:#16a34a;">
          <span>Advance Paid</span>
          <span style="font-weight:600;">− ₹${advanceAmount.toLocaleString()}</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;margin-top:4px;">
          <span style="font-size:14px;font-weight:600;color:#15803d;">Balance Due</span>
          <span style="font-size:18px;font-weight:800;color:#15803d;">₹${Math.max(finalCost - advanceAmount, 0).toLocaleString()}</span>
        </div>` : ''}
...
export const BillingPage: React.FC = () => {
  const { jobs, customers, devices, users, partRequests, inventory, updateJobStatus, currentUser, sales, markSalePaid } = useApp() as any;
                  ...
                  <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      ...
                      {(job.advanceAmount ?? 0) > 0 && (
                        <p className="text-[10px] text-green-600 font-medium mt-0.5">Adv: ₹{(job.advanceAmount).toLocaleString()}</p>
                      )}
                    </td>
                    <td className="px-4 py-4">
                      ...
                      {job.status === 'Completed' ? (
                        ...
                        <button
                          onClick={() => {
                            ...
                            printInvoice({ job, customer: c, device: d, engineer: e, approvedParts: ap, inventory, finalCost: job.actualCost ?? job.estimatedCost ?? 0, partsCost, serviceCharge, advanceAmount: job.advanceAmount ?? 0 });
                          }}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-600 text-[11px] font-medium rounded-lg uppercase tracking-wide hover:bg-gray-200 transition-colors"
                          title="Print Invoice"
                        >
