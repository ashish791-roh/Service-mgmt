...
import { saveTallySettingsToAPI, testTallyConnectionAPI, loadTallyStats, uploadTallyDocument, loadTallyDocuments, approveTallyDocument } from '../lib/tallySettings';
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <section className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="block">
                <span className="text-sm font-semibold text-slate-800">Mock mode</span>
                <div className="mt-2 flex items-center gap-3">
                  <input id="mock-mode" type="checkbox" checked={!!settings.mockMode} onChange={e => updateField('mockMode', e.target.checked)} />
                  <span className="text-sm text-slate-600">Enable mock Tally push (no Tally required)</span>
                </div>
              </label>
              
              <label className="block">
                <span className="text-sm font-semibold text-slate-800">Auto-push on approval</span>
                <div className="mt-2 flex items-center gap-3">
                  <input id="auto-push" type="checkbox" checked={!!settings.autoPushOnApproval} onChange={e => updateField('autoPushOnApproval', e.target.checked)} />
                  <span className="text-sm text-slate-600">Automatically push to Tally when approving documents</span>
                </div>
              </label>
            </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              <h3 className="text-sm font-semibold mb-3">Document Review</h3>
              {documents.length === 0 && <p className="text-sm text-slate-500">No documents to review.</p>}
              <div className="space-y-3">
                {documents.map(doc => (
                  <div key={doc.id} className="p-3 border rounded-lg bg-slate-50 flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-sm">{doc.fileName} <span className="text-xs text-slate-500">({doc.documentType})</span></p>
                      <p className="text-xs text-slate-500">Status: {doc.status} — Confidence: {doc.extractedData?.confidence?.toFixed?.(2) ?? '—'}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={async () => { const res = await approveTallyDocument(doc.id, 'approve'); if (res.ok) { setDocuments(prev => prev.map(d => d.id===doc.id ? { ...d, status: 'approved' } : d)); } }} className="px-3 py-1 rounded-md bg-emerald-500 text-white text-sm">Approve</button>
                      <button onClick={async () => { const res = await approveTallyDocument(doc.id, 'reject'); if (res.ok) { setDocuments(prev => prev.map(d => d.id===doc.id ? { ...d, status: 'rejected' } : d)); } }} className="px-3 py-1 rounded-md bg-rose-500 text-white text-sm">Reject</button>
                    </div>
                  </div>
                ))}
              </div>
