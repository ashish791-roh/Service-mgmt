...
const EngineerDetailModal = ({
  ...
  const getFilteredJobs = () => {
    ...
    if (type === 'overdue')   return jobs.filter(j => {
      ...
      const ageLevel = getJobAgeLevel(j.createdAt, j.status as any, device?.type);
      return ageLevel === 'red';
    });
    return [];
  };
...
const QRModal = ({ job, customer, device, onClose }: {
  ...
  React.useEffect(() => {
    generateQR(trackingUrl).then(url => { setQrDataUrl(url); setLoading(false); });
  }, [trackingUrl]);
...
async function generateQR(text: string): Promise<string> {
  try {
    ...
    if (ctx) {
      ...
      ctx.fillStyle = '#1f2937'; ctx.font = '12px monospace'; ctx.textAlign = 'center';
                ...
                if (!match) return (
                  <div className="flex items-start gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                    ...
                    <p className="text-[12px] font-medium text-gray-500">
                      This part has been <strong>not found in inventory</strong>. Your request will still be submitted for admin review.
                    </p>
                  </div>
                );
