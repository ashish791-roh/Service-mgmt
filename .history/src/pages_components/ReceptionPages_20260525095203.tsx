...
export const CustomersPage: React.FC = () => {
  ...
  const CustomerDetailModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[51] flex items-start justify-end bg-gray-900/40 backdrop-blur-sm" onClick={() => setSelectedCustomer(null)}>
        <div
          className="relative w-full max-w-xl h-full bg-white shadow-2xl flex flex-col overflow-hidden"
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-8 space-y-6">
      ...
      {summaryModal && (() => {
        ...
        const configs = {
          total:     { title: 'All Customers',    subtitle: `${customers.length} registered client${customers.length !== 1 ? 's' : ''}`,  accentColor: 'text-teal-600',  headerBg: 'bg-teal-50',  headerBorder: 'border-teal-100' },
          active:    { title: 'Active Jobs',       subtitle: `${activeJobs.length} job${activeJobs.length !== 1 ? 's' : ''} in progress`,   accentColor: 'text-amber-600', headerBg: 'bg-amber-50', headerBorder: 'border-amber-100' },
          completed: { title: 'Completed Jobs',    subtitle: `${completedJobs.length} successfully resolved`,                              accentColor: 'text-green-600', headerBg: 'bg-green-50', headerBorder: 'border-green-100' },
          revenue:   { title: 'Revenue Overview',  subtitle: 'Earnings breakdown from completed jobs',                                     accentColor: 'text-cyan-600',  headerBg: 'bg-cyan-50',  headerBorder: 'border-cyan-100' },
        };
        const cfg = configs[summaryModal];

        const jobStatusStyle: Record<string, { border: string; badge: string }> = {
          'New':         { border: 'border-l-cyan-400',   badge: 'bg-cyan-50 text-cyan-700 border border-cyan-200' },
          'Assigned':    { border: 'border-l-teal-400',   badge: 'bg-teal-50 text-teal-700 border border-teal-200' },
          'In Progress': { border: 'border-l-orange-400', badge: 'bg-orange-50 text-orange-700 border border-orange-200' },
          'Completed':   { border: 'border-l-green-400',  badge: 'bg-green-50 text-green-700 border border-green-200' },
          'Delivered':   { border: 'border-l-emerald-400',badge: 'bg-emerald-50 text-emerald-700 border border-emerald-200' },
        };

        const renderContent = () => {
          /* ── Total Customers ── */
          if (summaryModal === 'total') {
            const withJobs    = customers.filter(c => jobs.some(j => j.customerId === c.id));
            const withoutJobs = customers.filter(c => !jobs.some(j => j.customerId === c.id));
            ...
            return (
              <>
                {/* Summary stats strip */}
                <div className={`grid grid-cols-3 divide-x divide-teal-100 ${cfg.headerBg} ${cfg.headerBorder} border-b`}>
                  {[
                    { label: 'Total', value: customers.length, color: 'text-teal-700' },
                    { label: 'With Jobs', value: withJobs.length, color: 'text-amber-600' },
                    { label: 'This Month', value: newThisMonth.length, color: 'text-green-600' },
                  ].map(s => (
                    <div key={s.label} className="flex flex-col items-center py-4 px-2">
                      <p className={`text-[20px] font-semibold ${s.color}`}>{s.value}</p>
                      <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{s.label}</p>
                    </div>
                  ))}
                </div>

                {/* Customers with active jobs */}
                {withJobs.length > 0 && (
                  <div className="px-6 py-2 bg-gray-50 border-b border-gray-100">
                    <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Active Clients</p>
                  </div>
                )}
                {withJobs.map(c => {
                  const cJobs    = jobs.filter(j => j.customerId === c.id);
                  const active   = cJobs.filter(j => !['Completed', 'Delivered'].includes(j.status));
                  const done     = cJobs.filter(j => ['Completed', 'Delivered'].includes(j.status));
                  const revenue  = done.reduce((s, j) => s + (j.actualCost ?? j.estimatedCost), 0);
                  const lastSeen = cJobs.reduce((latest, j) => new Date(j.createdAt) > new Date(latest) ? j.createdAt : latest, c.createdAt);
                  return (
                    <div key={c.id}
                      className="flex items-center gap-4 px-6 py-4 hover:bg-teal-50/40 transition-colors cursor-pointer border-l-4 border-l-teal-400 group"
                      onClick={() => { setSummaryModal(null); setSelectedCustomer(c); setDetailTab('jobs'); }}>
                      <div className="w-10 h-10 rounded-xl bg-teal-100 text-teal-700 flex items-center justify-center font-semibold text-[15px] shrink-0 border border-teal-200">
                        {c.name.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-gray-900 group-hover:text-teal-700 transition-colors">{c.name}</p>
                        <p className="text-[11px] text-gray-500 mt-0.5 flex items-center gap-1"><Phone size={9} />{c.phone}</p>
                        {c.address && <p className="text-[10px] text-gray-400 truncate flex items-center gap-1 mt-0.5"><MapPin size={9} />{c.address}</p>}
                      </div>
                      <div className="text-right shrink-0 space-y-1">
                        <div className="flex items-center justify-end gap-2">
                          {active.length > 0 && <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-100 text-amber-700">{active.length} active</span>}
                          {done.length > 0 && <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-green-100 text-green-700">{done.length} done</span>}
                        </div>
                        {revenue > 0 && <p className="text-[11px] font-semibold text-gray-700">₹{revenue.toLocaleString()}</p>}
                        <p className="text-[9px] text-gray-400">{new Date(lastSeen).toLocaleDateString('en-IN')}</p>
                      </div>
                      <ChevronRight size={14} className="text-gray-300 group-hover:text-teal-500 transition-colors shrink-0" />
                    </div>
                  );
                })}

                {/* Customers without jobs */}
                {withoutJobs.length > 0 && (
                  <>
                    <div className="px-6 py-2 bg-gray-50 border-b border-t border-gray-100">
                      <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">No Jobs Yet</p>
                    </div>
                    {withoutJobs.map(c => (
                      <div key={c.id}
                        className="flex items-center gap-4 px-6 py-3 hover:bg-gray-50 transition-colors cursor-pointer border-l-4 border-l-gray-200 group"
                        onClick={() => { setSummaryModal(null); setSelectedCustomer(c); setDetailTab('jobs'); }}>
                        <div className="w-9 h-9 rounded-xl bg-gray-100 text-gray-500 flex items-center justify-center font-semibold text-[14px] shrink-0">
                          {c.name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-medium text-gray-700 group-hover:text-gray-900">{c.name}</p>
                          <p className="text-[11px] text-gray-400 mt-0.5">{c.phone}</p>
                        </div>
                        <span className="text-[10px] text-gray-400 shrink-0">Registered {new Date(c.createdAt).toLocaleDateString('en-IN')}</span>
                        <ChevronRight size={14} className="text-gray-200 group-hover:text-gray-400 transition-colors shrink-0" />
                      </div>
                    ))}
                  </>
                )}
                {customers.length === 0 && <p className="px-6 py-12 text-[13px] text-gray-400 text-center">No customers registered yet.</p>}
              </>
            );
          }

          /* ── Active Jobs ── */
          if (summaryModal === 'active') {
            if (activeJobs.length === 0) return (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                <Briefcase size={32} className="mb-3 opacity-40" />
                <p className="text-[13px] font-medium">No active jobs right now</p>
                <p className="text-[11px] mt-1">All jobs are completed or delivered</p>
              </div>
            );

            const byStatus = ['New', 'Assigned', 'In Progress'].reduce((acc, s) => {
              acc[s] = activeJobs.filter(j => j.status === s).length;
              return acc;
            }, {} as Record<string, number>);

            const sorted = [...activeJobs].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());

            return (
              <>
                {/* Status breakdown */}
                <div className={`grid grid-cols-3 divide-x divide-amber-100 ${cfg.headerBg} ${cfg.headerBorder} border-b`}>
                  {[
                    { label: 'New',         value: byStatus['New'] ?? 0,         color: 'text-cyan-600' },
                    { label: 'Assigned',    value: byStatus['Assigned'] ?? 0,    color: 'text-teal-600' },
                    { label: 'In Progress', value: byStatus['In Progress'] ?? 0, color: 'text-orange-600' },
                  ].map(s => (
                ...
                <div className="px-6 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Oldest first — click to open customer</p>
                </div>
                ...
                {sorted.map(job => {
                  ...
                  const engineer = users.find(u => u.id === job.assignedEngineerId);
                  const daysOld  = Math.floor((Date.now() - new Date(job.createdAt).getTime()) / 86400000);
                  const { border, badge } = jobStatusStyle[job.status] ?? { border: 'border-l-gray-300', badge: 'bg-gray-50 text-gray-600 border border-gray-200' };
                  return (
                    <div key={job.id}
                      className={`flex flex-col gap-2 px-6 py-4 border-l-4 ${border} hover:bg-amber-50/30 transition-colors cursor-pointer group`}
                      onClick={() => { if (customer) { setSummaryModal(null); setSelectedCustomer(customer); setDetailTab('jobs'); } }}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-semibold text-gray-900 truncate group-hover:text-amber-700 transition-colors">{job.problemDescription}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[11px] font-medium text-teal-600 flex items-center gap-1">
                              <Users size={9} />{customer?.name ?? 'Unknown'}
                            </span>
                            <span className="text-gray-300">·</span>
                            {engineer
                              ? <span className="text-[11px] text-gray-500 flex items-center gap-1"><Wrench size={9} />{engineer.name}</span>
                              : <span className="text-[11px] text-orange-500 font-medium">⚠ Unassigned</span>}
                          </div>
                        </div>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold shrink-0 ${badge}`}>{job.status}</span>
                      </div>
                      <div className="flex items-center gap-3 text-[11px] text-gray-400">
                        <span className="flex items-center gap-1"><Calendar size={9} />{new Date(job.createdAt).toLocaleDateString('en-IN')}</span>
                        <span className={`font-semibold ${daysOld > 10 ? 'text-red-500' : daysOld > 5 ? 'text-amber-500' : 'text-gray-500'}`}>{daysOld}d old</span>
                        <span>·</span>
                        <span className="font-medium text-gray-600">₹{(job.actualCost ?? job.estimatedCost ?? 0).toLocaleString()}</span>
                        {customer?.phone && <span className="ml-auto flex items-center gap-1 text-gray-400"><Phone size={9} />{customer.phone}</span>}
                      </div>
                    </div>
                  );
                })}
          ...
          /* ── Completed Jobs ── */
          if (summaryModal === 'completed') {
            if (completedJobs.length === 0) return (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                <Briefcase size={32} className="mb-3 opacity-40" />
                <p className="text-[13px] font-medium">No completed jobs yet</p>
              </div>
            );

            const totalValue  = completedJobs.reduce((s, j) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0);
            const avgValue    = Math.round(totalValue / completedJobs.length);
            const deliveredCount = completedJobs.filter(j => j.status === 'Delivered').length;

            const sorted = [...completedJobs].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

            return (
              <>
                {/* Stats strip */}
                <div className={`grid grid-cols-3 divide-x divide-green-100 ${cfg.headerBg} ${cfg.headerBorder} border-b`}>
                  {[
                    { label: 'Total', value: completedJobs.length, color: 'text-green-700' },
                    { label: 'Delivered', value: deliveredCount, color: 'text-emerald-600' },
                    { label: 'Avg Value', value: `₹${avgValue.toLocaleString()}`, color: 'text-teal-600' },
                  ].map(s => (
                    <div key={s.label} className="flex flex-col items-center py-4">
                ...
                <div className="px-6 py-2 bg-gray-50 border-b border-gray-100">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Most recent first — click to open customer</p>
                </div>
                ...
                {sorted.map(job => {
                  ...
                  return (
                    <div key={job.id}
                      className={`flex flex-col gap-2 px-6 py-4 border-l-4 ${border} hover:bg-green-50/30 transition-colors cursor-pointer group`}
                      onClick={() => { if (customer) { setSummaryModal(null); setSelectedCustomer(customer); setDetailTab('jobs'); } }}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-semibold text-gray-900 truncate group-hover:text-green-700 transition-colors">{job.problemDescription}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[11px] font-medium text-teal-600 flex items-center gap-1"><Users size={9} />{customer?.name ?? 'Unknown'}</span>
                            {engineer && <><span className="text-gray-300">·</span><span className="text-[11px] text-gray-500 flex items-center gap-1"><Wrench size={9} />{engineer.name}</span></>}
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${badge}`}>{job.status}</span>
                          <p className="text-[12px] font-bold text-green-700">₹{(job.actualCost ?? job.estimatedCost ?? 0).toLocaleString()}</p>
                        </div>
                      </div>
                      <p className="text-[11px] text-gray-400 flex items-center gap-1"><Calendar size={9} />{new Date(job.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                    </div>
                  );
                })}
              </>
          ...
          if (summaryModal === 'revenue') {
            const totalRevenue = completedJobs.reduce((s, j) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0);
            const byCustomer = customers.map(c => {
              const cDone = jobs.filter(j => j.customerId === c.id && ['Completed', 'Delivered'].includes(j.status));
              return { ...c, revenue: cDone.reduce((s, j) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0), count: cDone.length };
            ...
            const monthRevenue = completedJobs
              .filter(j => new Date(j.createdAt) >= thisMonth)
              .reduce((s, j) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0);

            return (
              <>
                {/* Revenue stats */}
                <div className={`grid grid-cols-3 divide-x divide-cyan-100 ${cfg.headerBg} ${cfg.headerBorder} border-b`}>
                  {[
                    { label: 'Total', value: `₹${(totalRevenue / 1000).toFixed(1)}k`, color: 'text-cyan-700' },
                    { label: 'This Month', value: `₹${(monthRevenue / 1000).toFixed(1)}k`, color: 'text-teal-600' },
                    { label: 'Customers', value: byCustomer.length, color: 'text-amber-600' },
                  ].map(s => (
                    <div key={s.label} className="flex flex-col items-center py-4">
                      <p className={`text-[18px] font-semibold ${s.color}`}>{s.value}</p>
                      <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mt-0.5">{s.label}</p>
                ...
                <div className="px-6 py-2 bg-gray-50 border-b border-gray-100">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Top earners — click to view customer</p>
                </div>
                ...
                {byCustomer.map((c, idx) => {
                  ...
                  return (
                    <div key={c.id}
                      className="flex items-center gap-4 px-6 py-4 hover:bg-cyan-50/30 border-b border-gray-100 transition-colors cursor-pointer group"
                      onClick={() => { setSummaryModal(null); setSelectedCustomer(c); setDetailTab('jobs'); }}>
                      <span className={`text-[11px] font-bold w-5 text-center shrink-0 ${idx === 0 ? 'text-amber-500' : idx === 1 ? 'text-gray-400' : idx === 2 ? 'text-amber-700' : 'text-gray-300'}`}>
                        {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `#${idx + 1}`}
                      </span>
                      <div className="w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center text-[12px] font-semibold shrink-0">
                        {c.name.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between mb-1.5">
                          <p className="text-[12px] font-semibold text-gray-900 truncate group-hover:text-teal-700 transition-colors">{c.name}</p>
                          <p className="text-[12px] font-bold text-cyan-700 shrink-0 ml-2">₹{c.revenue.toLocaleString()}</p>
                        </div>
                        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-teal-400 to-cyan-500 rounded-full transition-all" style={{ width: `${pct}%` }} />
                        </div>
                        <p className="text-[10px] text-gray-400 mt-1">{c.count} completed job{c.count !== 1 ? 's' : ''} · {pct}% of top</p>
                      </div>
                      <ChevronRight size={14} className="text-gray-200 group-hover:text-teal-400 transition-colors shrink-0" />
                    </div>
                  );
                })}
                {byCustomer.length === 0 && <p className="px-6 py-12 text-[13px] text-gray-400 text-center">No revenue data yet.</p>}
              </>
            );
          }
          return null;
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-start justify-end bg-gray-900/40 backdrop-blur-sm" onClick={() => setSummaryModal(null)}>
            <div className="relative max-w-xl h-full bg-white shadow-2xl flex flex-col overflow-hidden" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200 bg-white shrink-0">
                <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.map((c) => {
          ...
          const completedJobs = cJobs.filter(j => ['Completed', 'Delivered'].includes(j.status));
          const totalSpend = completedJobs.reduce((s, j) => s + (j.actualCost ?? j.estimatedCost), 0);
          const lastJobDate = cJobs.length > 0 
            ? new Date(cJobs.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0].createdAt)
            ...
            >
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] font-normal text-gray-500 flex items-center gap-1"><Phone size={10} /> {c.phone}</p>
                  {c.address && <p className="text-[10px] font-normal text-gray-400 truncate flex items-center gap-1 mt-0.5"><MapPin size={10} /> {c.address}</p>}
                </div>
                <button
              ...
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] font-semibold text-gray-700">{lastJobDate.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</p>
                </div>
              </div>
              ...
              {cJobs.length > 0 && (
                <div className="flex items-center justify-between">
                  <div className="flex gap-1.5 flex-wrap">
                    {cJobs.slice(0, 3).map(j => <StatusBadge key={j.id} status={j.status} />)}
                    {cJobs.length > 3 && (
                      <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-gray-100 text-gray-600">+{cJobs.length - 3}</span>
                    )}
                  </div>
