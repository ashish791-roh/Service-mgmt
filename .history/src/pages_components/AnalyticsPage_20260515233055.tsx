...
import {
  ...
  X, ChevronRight, Calendar, TrendingUp, ShoppingCart, Package,
  BarChart2,
} from 'lucide-react';
...
function filterSalesByPeriod(sales: any[], period: TimePeriod): any[] {
  const start = getStartDate(period);
  if (!start) return sales;
  return sales.filter(s => new Date(s.createdAt) >= start);
}

// ── SVG Bar Chart (pure SVG, no external deps) ────────────────────────────────
...
const ChartsSection: React.FC<{
  ...
  period: TimePeriod;
  periodLabel: string;
}> = ({ jobs, sales, partRequests, inventory, period, periodLabel }) => {
  const now = new Date();
  ...
  const partsWithCost = approvedParts.map(r => {
    const invItem = inventory.find((inv: any) => inv.name?.toLowerCase() === r.partName?.toLowerCase());
          ...
          {topParts.length === 0 ? (
            ...
            <div className="space-y-3">
              {topParts.map(([name, qty]) => {
                ...
                const invItem = inventory.find((inv: any) => inv.name?.toLowerCase() === name.toLowerCase());
                const unitCost = invItem?.unitCost ?? 0;
                return (
...
export const AnalyticsPage: React.FC = () => {
  ...
  // Job revenue = actualCost (or estimatedCost) of Completed/Delivered jobs
  ...
  // Sales revenue = sum of totalAmount on Sale records
  const salesRevenue = useMemo(
    () => filteredSales.reduce((s, sale) => s + (sale.totalAmount ?? 0), 0),
    [filteredSales],
  );

  // Total revenue = jobs + sales (ONE consistent total used everywhere)
