...
const DayWiseActivity = ({ jobs, sales }: { jobs: any[]; sales: any[] }) => {
  ...
  const activityMap = useMemo(() => {
    const result: Record<string, { jobs: any[]; sales: any[]; revenue: number }> = {};
    days.forEach(d => {
...
export const AdminDashboard: React.FC<{ onNavigate: (p: string) => void }> = ({ onNavigate }) => {
  const { jobs, users, customers, partRequests, inventory, sales, stats } = useApp();
...
const DetailModal = ({ type, onClose, jobs, users, partRequests, sales }: any) => {
      ...
      <div className="relative w-full max-w-xl h-full bg-white shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          <div className="divide-y divide-gray-100 mt-4">
            {type === 'completed' && jobs.filter((j: any) => ['Completed', 'Delivered'].includes(j.status)).map((j: any) => (
            ...
            {type === 'pending' && jobs.filter((j: any) => ['New', 'Assigned', 'In Progress'].includes(j.status)).map((j: any) => (
              <div key={j.id} className="py-3 flex justify-between items-center text-[13px]">
                <p className="font-medium text-gray-900 truncate max-w-[300px]">{j.problemDescription}</p>
                <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-[11px] font-medium">{j.status}</span>
              </div>
            ))}
            {type === 'engineers' && users.filter((u: any) => u.role === 'engineer').map((u: any) => (
            ...
            {type === 'parts' && partRequests.map((r: any) => (
            ...
            {type === 'revenue' && (
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl">
                  ...
                  <p className="text-[24px] font-bold text-emerald-800 mt-1">₹{jobs.filter((j: any) => ['Completed', 'Delivered'].includes(j.status)).reduce((s: number, j: any) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0).toLocaleString()}</p>
                </div>
                <div className="p-4 bg-purple-50 border border-purple-100 rounded-xl">
                  <p className="text-[11px] font-medium text-purple-600 uppercase tracking-wide">Product Sales</p>
                  <p className="text-[24px] font-bold text-purple-800 mt-1">₹{sales.reduce((s: number, sale: any) => s + (sale.totalAmount ?? 0), 0).toLocaleString()}</p>
                </div>
              </div>
            )}
          </div>
