...
import { Wrench, CheckCircle, Banknote, Users, Activity, Target, X, ChevronRight, Calendar, TrendingUp } from 'lucide-react';
...
type TimePeriod = '7d' | '30d' | '90d' | 'overall';
...
function filterJobsByPeriod(jobs: any[], period: TimePeriod): any[] {
...
const MonthWiseAnalytics: React.FC<{ jobs: any[]; customers: any[] }> = ({ jobs, customers }) => {
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    ...
    const newCustomerIds = new Set(monthJobs.map(j => j.customerId));
    return {
...
export const AnalyticsPage: React.FC = () => {
  ...
  // ── Filtered jobs based on selected time period ───────────────────────────
  const filteredJobs = useMemo(() => filterJobsByPeriod(jobs, timePeriod), [jobs, timePeriod]);

  const totalRevenue = filteredJobs.filter(j => ['Completed', 'Delivered'].includes(j.status))
    ...
    <div className="max-w-[1400px] mx-auto pb-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-2 text-gray-500 shrink-0">
          ...
          <span className="text-[13px] font-medium text-gray-600">Past Analytics</span>
        </div>
        <div className="flex flex-wrap gap-2">
      ...
      {/* KPI Cards Grid — all clickable, filtered by time period */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Total Revenue"  value={`₹${(totalRevenue/1000).toFixed(1)}k`}                                          icon={Banknote}     color="green"  sub={periodLabels[timePeriod]}  onClick={() => setActiveModal('revenue')} />
        <MetricCard title="Total Volume"   value={filteredJobs.length}                                                             icon={Wrench}       color="cyan"   sub={periodLabels[timePeriod]}  onClick={() => setActiveModal('volume')} />
      ...
      {(() => {
        ...
        const pendingJobs = filteredJobs.filter(j => ['New', 'Assigned', 'In Progress'].includes(j.status));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card onClick={() => setActiveModal('keymetrics')}>
          ...
          <div className="p-6 space-y-4">
            {[
              { label: 'Completion Rate', value: `${filteredJobs.length > 0 ? Math.round((filteredJobs.filter(j => ['Completed', 'Delivered'].includes(j.status)).length / filteredJobs.length) * 100) : 0}%`, color: 'text-green-600' },
              { label: 'Avg. Ticket Size', value: `₹${filteredJobs.length > 0 ? Math.round(filteredJobs.reduce((s, j) => s + j.estimatedCost, 0) / filteredJobs.length).toLocaleString() : 0}`, color: 'text-teal-600' },
      ...
      <MonthWiseAnalytics jobs={jobs} customers={customers} />
