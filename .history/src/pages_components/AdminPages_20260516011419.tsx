...
import { CheckCircle, Banknote, Hourglass, Users, UserPlus, ChevronRight, Package, X, Pencil, Trash2, Shield, Search, Filter, ChevronLeft, Download, RefreshCw, Clock, User, Database, AlertCircle, CalendarDays, TrendingUp, Activity } from 'lucide-react';
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <MetricCard title="Total Revenue" value={`₹${((jobs.filter(j => ['Completed', 'Delivered'].includes(j.status)).reduce((s, j) => s + (j.actualCost ?? j.estimatedCost ?? 0), 0) + sales.reduce((s, sale) => s + (sale.totalAmount ?? 0), 0)) / 1000).toFixed(1)}k`} icon={Banknote} color="green" sub="All Time"   onClick={() => setActiveModal('revenue')} />
      </div>
