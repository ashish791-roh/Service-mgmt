...
const DayWiseActivity: React.FC<{ jobs: any[]; sales: any[] }> = ({ jobs, sales }) => {
  ...
  const weekKey = (d: Date) => toKey(d);
  const weekDays = days.slice(0, 7);
  const weekJobs = weekDays.reduce((s, d) => s + (activityMap[weekKey(d)]?.jobs.length ?? 0), 0);
  const weekRevenue = weekDays.reduce((s, d) => s + (activityMap[weekKey(d)]?.revenue ?? 0), 0);
  const weekSales = weekDays.reduce((s, d) => s + (activityMap[weekKey(d)]?.sales.length ?? 0), 0);
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-cyan-50 border border-cyan-100 rounded-xl p-4 text-center">
            <p className="text-[22px] font-bold text-cyan-700">{weekJobs}</p>
            <p className="text-[11px] font-medium text-cyan-600 uppercase tracking-wide mt-0.5">Jobs This Week</p>
          </div>
          <div className="bg-green-50 border border-green-100 rounded-xl p-4 text-center">
            <p className="text-[22px] font-bold text-green-700">₹{(weekRevenue / 1000).toFixed(1)}k</p>
            <p className="text-[11px] font-medium text-green-600 uppercase tracking-wide mt-0.5">Revenue This Week</p>
          </div>
          <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 text-center">
            <p className="text-[22px] font-bold text-purple-700">{weekSales}</p>
            <p className="text-[11px] font-medium text-purple-600 uppercase tracking-wide mt-0.5">Sales This Week</p>
          </div>
        </div>
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard title="Client Database"     value={customers.length}    icon={Users}        color="teal"   sub="Registered"  onClick={() => setActiveModal('clients')} />
      </div>
