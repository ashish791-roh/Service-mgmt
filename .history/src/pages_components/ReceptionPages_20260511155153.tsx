...
import { Briefcase, AlertCircle, Zap, Users, Settings, Search, Plus, ArrowRight, X, Phone, MapPin, Monitor, Wrench, Calendar, ChevronRight, QrCode, Download, Printer, Package, Minus, ChevronDown } from 'lucide-react';
...
export const CustomersPage: React.FC = () => {
  const { customers, devices, jobs, addCustomer, addDevice, addJob, users, inventory } = useApp();
  ...
  const [selectedParts, setSelectedParts] = useState<{ id: string; name: string; unitCost: number; quantity: number }[]>([]);
  ...
  const handleNext = async () => {
    ...
    if (step === 1) {
      ...
      if (!jobForm.problemDescription) { show('Problem description is required', 'error'); return; }
      if (!jobForm.estimatedCost || parseFloat(jobForm.estimatedCost) === 0) { show('Please add at least a service charge or a part to estimate the bill', 'error'); return; }
      setSubmitting(true);
      try {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-8 shadow-lg overflow-hidden relative">
            ...
            <div className="space-y-4 mb-8">
              {step === 1 && (
                <>
                  ...
                  <div>
                    ...
                    <input className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2 text-[13px] font-medium text-gray-900 focus:outline-none focus:border-teal-500 focus:bg-white transition-colors" name="tel" autoComplete="tel" inputMode="numeric" value={custForm.phone} onChange={e => setCustForm({ ...custForm, phone: e.target.value })} placeholder="Mobile Number" />
                  </div>
                  <div>
              ...
              {step === 3 && (() => {
                // ── Bill calculator helpers ──────────────────────────────────
                const partsCostTotal = selectedParts.reduce((s, p) => s + p.unitCost * p.quantity, 0);
                const serviceChargeVal = parseFloat(serviceCharge) || 0;
                const estimatedTotal = partsCostTotal + serviceChargeVal;

                const filteredInventory = inventory.filter(inv =>
                  inv.quantity > 0 &&
                  inv.name.toLowerCase().includes(partSearch.toLowerCase()) &&
                  !selectedParts.find(sp => sp.id === inv.id)
                );

                const addPart = (inv: any) => {
                  setSelectedParts(prev => [...prev, { id: inv.id, name: inv.name, unitCost: inv.unitCost, quantity: 1 }]);
                  setPartSearch('');
                  setShowPartDropdown(false);
                };

                const removePart = (id: string) => setSelectedParts(prev => prev.filter(p => p.id !== id));

                const changeQty = (id: string, qty: number) => {
                  if (qty < 1) { removePart(id); return; }
                  setSelectedParts(prev => prev.map(p => p.id === id ? { ...p, quantity: qty } : p));
                };

                // Sync estimatedCost into jobForm for submission
                if (jobForm.estimatedCost !== String(estimatedTotal)) {
                  setJobForm(f => ({ ...f, estimatedCost: String(estimatedTotal) }));
                }
                ...
                return (
                  <>
                    ...
                    <div>
                      ...
                      <textarea className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2 text-[13px] font-medium text-gray-900 focus:outline-none focus:border-teal-500 focus:bg-white transition-colors resize-none" rows={3} value={jobForm.problemDescription} onChange={e => setJobForm({ ...jobForm, problemDescription: e.target.value })} placeholder="Describe the problem in detail..." />
                    </div>

                    {/* Parts picker */}
                    <div>
                      <label className="block text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1.5">Parts Required (Optional)</label>
                      <div className="relative">
                        <div className="flex items-center gap-2 w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 focus-within:border-teal-500 focus-within:bg-white transition-colors">
                          <Package size={14} className="text-gray-400 shrink-0" />
                          <input
                            className="flex-1 bg-transparent text-[13px] font-medium text-gray-900 outline-none placeholder-gray-400"
                            value={partSearch}
                          ...
                          <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            {filteredInventory.slice(0, 8).map(inv => (
                              <button
                                key={inv.id}
                                type="button"
                                onMouseDown={() => addPart(inv)}
                                className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-teal-50 text-left transition-colors"
                              >
                                <span className="text-[13px] font-medium text-gray-900">{inv.name}</span>
                                <span className="text-[12px] text-gray-500">₹{inv.unitCost.toLocaleString()} · {inv.quantity} in stock</span>
                              </button>
                            ))}
                          </div>
                        )}
                      ...
                      {selectedParts.length > 0 && (
                        <div className="mt-2 space-y-2">
                          {selectedParts.map(p => (
                            <div key={p.id} className="flex items-center gap-3 bg-orange-50 border border-orange-100 rounded-lg px-3 py-2">
                              <Package size={13} className="text-orange-400 shrink-0" />
                              <span className="flex-1 text-[13px] font-medium text-gray-800">{p.name}</span>
                              <span className="text-[12px] text-gray-500">₹{p.unitCost.toLocaleString()}</span>
                              <div className="flex items-center gap-1">
                                <button type="button" onClick={() => changeQty(p.id, p.quantity - 1)} className="w-6 h-6 flex items-center justify-center rounded bg-white border border-gray-200 hover:bg-red-50 hover:border-red-200 transition-colors">
                                  <Minus size={11} className="text-gray-600" />
                                </button>
                                <span className="w-6 text-center text-[13px] font-semibold text-gray-900">{p.quantity}</span>
                                <button type="button" onClick={() => changeQty(p.id, p.quantity + 1)} className="w-6 h-6 flex items-center justify-center rounded bg-white border border-gray-200 hover:bg-teal-50 hover:border-teal-200 transition-colors">
                                  <Plus size={11} className="text-gray-600" />
                                </button>
                              </div>
                              <span className="text-[13px] font-semibold text-orange-600 w-16 text-right">₹{(p.unitCost * p.quantity).toLocaleString()}</span>
                              <button type="button" onClick={() => removePart(p.id)} className="text-gray-400 hover:text-red-500 transition-colors">
                                <X size={14} />
                              </button>
                            </div>
                          ))}
                    ...
                    {/* Service charge */}
                    <div>
                      <label className="block text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1.5">Service Charge (₹)</label>
                      <div className="flex items-center gap-2 w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 focus-within:border-teal-500 focus-within:bg-white transition-colors">
                        ...
                        <input
                          ...
                          placeholder="Labour / inspection fee..."
                        />
                      </div>
                    </div>

                    {/* Live bill summary */}
                    <div className="bg-gradient-to-br from-teal-50 to-teal-50/40 border border-teal-100 rounded-xl p-4 space-y-2">
                      <p className="text-[11px] font-semibold text-teal-700 uppercase tracking-wide mb-3">Estimated Bill Summary</p>
                      <div className="flex justify-between text-[13px]">
                        <span className="text-gray-600 flex items-center gap-1.5"><Package size={12} className="text-orange-400" /> Parts Cost</span>
                        <span className="font-medium text-gray-900">₹{partsCostTotal.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-[13px]">
                        <span className="text-gray-600 flex items-center gap-1.5"><Wrench size={12} className="text-teal-500" /> Service Charge</span>
                        <span className="font-medium text-gray-900">₹{serviceChargeVal.toLocaleString()}</span>
                      </div>
                      <div className="border-t border-teal-200 pt-2 flex justify-between">
                        <span className="text-[14px] font-semibold text-gray-800">Total Estimate</span>
                        <span className="text-[16px] font-bold text-teal-700">₹{estimatedTotal.toLocaleString()}</span>
                      </div>
                    </div>
